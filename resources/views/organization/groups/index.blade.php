...
<div class="card mb-5 mb-xl-8">
    ...
    <div class="card-header border-0">
        <div class="card-toolbar">
            <a href="{{ route('organizationgroups.create', $org->slug) }}" class="btn btn-sm btn-primary">
            ...
            <table class="table align-middle gs-0 gy-4">
                ...
                @foreach ( $groups as $group )
                    <tr>
                        <td>
                            <div class="d-flex align-items-center">
                                <div class="d-flex justify-content-start flex-column">
                                    <span  class="text-dark fw-bolder mb-1 fs-6">{{ $group->id }}</span>
                                </div>
                            </div>
                        </td>
                        <td>
                            <span class="text-dark fw-bolder d-block mb-1 fs-6">{{ $group->name }}</span>
                        </td>
                        <td>
                            <span class="text-dark fw-bolder d-block mb-1 fs-6">{{ $group->created_at->format('d/m/Y') }}</span>
                        </td>
                            ...
                            <span class="text-dark fw-bolder d-block mb-1 fs-6">
                                ...
                                @isset($group->users)
                                    ...
                                    @if( !empty($receiver_array) )
                                        @foreach($receiver_array as $id)
                                            <?php
                                            $user = \App\Models\Receiver::find($id);
                                            if ( $user->org_id == $group->org_id )
                                                $user_list[] = $user->first_name . ' ' . $user->last_name;
                                            ?>
                                        @endforeach
                                        <p style="color: green;">Receivers:<br>
                                            {{ collect($user_list)->implode(', ') }}</p>
                                    @endif


                                    @if( !empty($tipper_array) )
                                        @foreach($tipper_array as $id)
                                            <?php
                                            $user = \App\Models\Tipper::find($id);
                                            $tipper_list[] = $user->first_name . ' ' . $user->last_name;
                                            ?>
                                        @endforeach
                                        <p style="color:red;">Tippers:<br>
                                            {{ collect($tipper_list)->implode(', ') }}</p>
                                    @endif
                                @endisset
                        ...
                        <td class="text-end">
                            <form method="POST" action="{{ route('organizationgroups.destroy', [$org->slug, $group]) }}">
                                <a href="{{ route('organizationgroups.show', [$org->slug, $group]) }}" class="btn btn-icon btn-bg-light btn-active-color-primary bg-warning btn-sm me-1">
                                ...
                                <a href="{{ route('organizationgroups.edit', [$org->slug, $group]) }}" class="btn btn-icon btn-bg-light btn-active-color-primary bg-warning btn-sm me-1">
                                    <!--begin::Svg Icon | path: icons/duotone/Communication/Write.svg-->
                                    <span class="svg-icon svg-icon-3">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
